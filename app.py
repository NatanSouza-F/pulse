"""
Pulse — Demo Dashboard v3 Turbinado
O pulso do seu negócio, no seu bolso.

Novidades v3:
  - Carrossel 3D animado na tela inicial (estilo Argus login)
  - KPI cards premium com glow verde (estilo dashboard Argus)
  - Ícones flutuantes do topo removidos
  - Fonte Inter 100% unificada (inclusive em gráficos Plotly)
  - Polish mobile completo
"""
...
# Config Plotly global — fonte Inter em TODOS os gráficos
import plotly.io as pio
pio.templates["pulse_theme"] = go.layout.Template(
    layout=go.Layout(
...
pio.templates.default = "pulse_theme"


# ═══════════════════════════════════════════════════════════════
# CSS GLOBAL — FONTE INTER UNIFICADA, SEM ÍCONES FLUTUANTES
# ═══════════════════════════════════════════════════════════════
st.markdown("""
<style>
    @import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800;900&display=swap');

    /* Inter em absolutamente tudo */
    html, body, [class*="css"], *, button, input, select, textarea,
    .stApp, .block-container, [data-testid], .stTabs, .stMetric,
    .js-plotly-plot, .plotly, .main-svg text, .plot-container,
    .stMarkdown, h1, h2, h3, h4, h5, h6, p, span, div, a, label {
    ...
    /* ═══════════════════════════════════════════════════════════════
       CARROSSEL 3D ANIMADO (ESTILO LOGIN ARGUS)
       ═══════════════════════════════════════════════════════════════ */
    .carousel-wrapper {
        position: relative;
        width: 100%;
    ...
    @keyframes carousel-rotate {
        0%, 100% {
            opacity: 0;
            transform: translateX(0) rotateY(90deg) scale(0.8);
        }
        5% {
            opacity: 1;
    ...
    /* ═══════════════════════════════════════════════════════════════
       KPI CARDS PREMIUM COM GLOW (ESTILO DASHBOARD ARGUS)
       ═══════════════════════════════════════════════════════════════ */
...
def layout_chart(altura=280, yaxis_opts=None):
    base = {
        ...
        "xaxis": {
            "gridcolor": "#e2e8f0",
            "tickfont": {"family": "Inter, sans-serif", "color": "#0f172a", "size": 10},
            "title": {"font": {"family": "Inter, sans-serif", "color": "#0f172a"}},
        },
        "yaxis": {
            "gridcolor": "#e2e8f0",
            "tickfont": {"family": "Inter, sans-serif", "color": "#0f172a", "size": 10},
        ...
        "height": altura,
        "hoverlabel": {"bgcolor": "white", "font": {"family": "Inter, sans-serif", "color": "#0f172a"}},
        "legend": {"font": {"family": "Inter, sans-serif", "color": "#0f172a", "size": 11}},
    }
    if yaxis_opts:
...
# ═══════════════════════════════════════════════════════════════
# CARROSSEL 3D ANIMADO (SÓ TELA INICIAL)
# ═══════════════════════════════════════════════════════════════
        ...
        <div class="carousel-track">
            <div class="carousel-card">
                ...
                <div>
                    ...
                    <div class="carousel-delta">▲ +12% este mês</div>
                </div>
            </div>
            <div class="carousel-card">
                <div style="display: flex; justify-content: space-between; align-items: center;">
                ...
                <div>
                    ...
                    <div class="carousel-delta">▲ +8,3% vs anterior</div>
                </div>
            </div>
            <div class="carousel-card">
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <span class="carousel-label">Cross-Sell</span>
                    <span class="carousel-icon">🔄</span>
                </div>
                <div>
                    <div class="carousel-value">34,7%</div>
                    <div class="carousel-delta">▲ +5,1pp últimos 30d</div>
                </div>
            </div>
            <div class="carousel-card">
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <span class="carousel-label">Satisfação (NPS)</span>
                    <span class="carousel-icon">⭐</span>
                </div>
                <div>
...
def gerar_dados_contabil(mes_ref="Jun", ano_ref="2026"):
    ...
    for i, mes in enumerate(MESES):
        ...
        prod.append({"Mes": mes, "Horas_Fechamento": round(horas, 1),
                     "Receita_Honorarios": round(receita, 2)})
...
def tela_inicial():
    ...
    st.markdown(
        '<p style="text-align:center; color:#475569; margin-top:4px; margin-bottom:8px; font-weight:500;">'
        'O pulso do seu negócio, no seu bolso.</p>',
        unsafe_allow_html=True
    )

    # Carrossel animado (estilo Argus login)
    carrossel_animado()

    st.markdown("""
    <p style="text-align:center; color:#0f172a; font-size:0.92rem; font-weight:500; margin-top:4px;">
    ...
    perfis = [
        ("🛡️", "Sou Corretora de Seguros",
         "Carteira, comissões, renovações e alertas.", "corretora"),
    ...
    for icon, titulo, desc, perfil in perfis:
        ...
        <div class="perfil-card">
            ...
            <div style="font-weight:700; font-size:1.05rem; color:#0f172a; margin-top:4px;">{titulo}</div>
            <div style="font-size:0.85rem; color:#475569; line-height:1.4;">{desc}</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    <div style="text-align:center; margin-top:32px; padding-top:20px;
                border-top:1px solid #e2e8f0; color:#64748b; font-size:0.75rem;">
...
def dash_corretora():
    ...
    <div class="dash-header">
        <div>
            ...
            <div class="dash-subtitle">Demo para Corretoras</div>
        </div>
    ...
    <div class="insight-box">
        💡 <strong>Insight de {mes}/{ano}:</strong> Sua taxa de renovação está 8% acima da média.
        O ticket médio subiu devido ao aumento de apólices de Vida.<br>
    ...
    t1, t2, t3, t4, t5 = st.tabs([
        "📈 Evolução", "🔔 Alertas", "🎯 Mix", "📋 Carteira", "🏆 Top"
    ])
    ...
    with t1:
        ...
        fig2.add_trace(go.Bar(name="Novos", x=df_evo["Mes"],
                              y=df_evo["Novos_Contratos"], marker_color=VERDE))
    ...
    with t5:
        ...
        fig = go.Figure(go.Bar(
            x=top_clientes.values, y=top_clientes.index, orientation="h",
...
def dash_contabil():
    ...
    <div class="dash-header">
        <div>
            ...
            <div class="dash-subtitle">Demo para Escritórios</div>
        </div>
        <span class="demo-badge">DEMO</span>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown(f"""
    <div class="insight-box">
        💡 <strong>Insight de {mes}/{ano}:</strong> 82% das entregas dentro do prazo.
        Clientes do Simples com maior risco de atraso.
    </div>
    """, unsafe_allow_html=True)
    ...
    t1, t2, t3, t4, t5, t6 = st.tabs([
        "📈 Produtividade", "⚠️ Atrasados", "📋 Carteira", "🥧 Regimes", "💰 Top", "📅 Obrigações"
    ])
...
def dash_clinica():
        ...
        if not agenda.empty:
            for _, row in agenda.iterrows():
                ...
                <div class="alerta-item ok">
                    <div style="display:flex; justify-content:space-between;">
                        <div>
                            <strong style="color:#0f172a;">{row['Paciente']}</strong><br>
                            <span style="font-size:0.78rem; color:#475569;">
    ...
    with t4:
        ...
        fig = px.bar(perf, x="Profissional", y="Valor", text="Valor",
    ...
    with t6:
        ...
        fig = px.pie(pgto, values="Qtd", names="Forma", hole=0.5,
                     color_discrete_sequence=[VERDE, AZUL, ROXO])
        fig.update_layout(**layout_chart(300))
        st.plotly_chart(fig, use_container_width=True)
...
def dash_barbearia():
    ...
    with t2:
        ...
        fig = go.Figure(go.Bar(
            ...
            marker_color=VERDE, text=top_prod.values, textposition='outside',
            texttemplate='%{text:.2s}'
        ))
        fig.update_layout(**layout_chart(260, yaxis_opts={
            "tickfont": {"family": "Inter, sans-serif", "color": "#0f172a", "size": 10}
        ...
        if not agenda.empty:
            for _, row in agenda.iterrows():
                ...
                <div class="alerta-item ok">
                    <div style="display:flex; justify-content:space-between;">
                        <div>
                            <strong style="color:#0f172a;">{row['Cliente']}</strong><br>
                            <span style="font-size:0.78rem; color:#475569;">
