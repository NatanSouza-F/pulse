...
<style>
    ...
    /* Background navy com gradiente sutil */
    .stApp {
        background:
...
def layout_chart(altura=280, yaxis_opts=None, showlegend=True):
    base = {
        ...
        "height": altura,
        "hoverlabel": {"bgcolor": "#1a2e42", "font": {"family": "Inter, sans-serif", "color": TEXTO}, "bordercolor": VERDE_NEON},
...
def kpi_com_comparacao(label, valor_atual, valor_anterior, formatter=None, label_comp="vs ant"):
    ...
    <div style="background: linear-gradient(135deg, #1a2e42 0%, #243b54 100%);
        ...
        <div style="color: #94a3b8; font-size: 0.72rem; margin-top: 4px;">
            {label_comp}: <strong style="color: #cbd5e1;">{formatter(valor_anterior) if valor_anterior else '—'}</strong>
            {badge}
        </div>
    </div>
    """, unsafe_allow_html=True)
...
def gerar_insight(tipo, dados):
    ...
    if tipo == "corretora":
        ...
        return gerar_insight_contabil(dados)
    elif tipo == "clinica":
        return gerar_insight_clinica(dados)
    elif tipo == "barbearia":
        return gerar_insight_barbearia(dados)
...
def gerar_insight_corretora(dados):
    ...
    if vencendo > 0:
        ...
        insights.append(
            ...
            f"aproximadamente {formatar_brl(receita_em_risco)}/mês em risco."
        )
    if comissao_ant and comissao > comissao_ant:
        pct = ((comissao - comissao_ant) / comissao_ant) * 100
        insights.append(
            f"📈 Comissão cresceu <strong>{pct:.1f}%</strong> vs período anterior. "
            f"Tendência positiva sustentada."
        )
    if not insights:
...
def gerar_insight_contabil(dados):
    ...
    if atrasados > 0:
        ...
        insights.append(
            f"⚠️ <strong>{atrasados} clientes em atraso</strong> ({pct_atraso:.1f}% da carteira). "
            f"Revisar processo de coleta de documentos."
        )
    if pct_prazo >= 80:
        insights.append(
            f"✅ <strong>{pct_prazo:.0f}% das entregas no prazo</strong> — "
            f"acima da média do setor (72%)."
        )
    elif pct_prazo < 70:
        insights.append(
            f"🎯 Meta de entregas no prazo: 80%. Atual: {pct_prazo:.0f}%. "
            f"Gap: {80 - pct_prazo:.0f}pp."
        )
...
def gerar_insight_clinica(dados):
    ...
    if receita_ant and receita > receita_ant:
        ...
        insights.append(
            f"💚 Receita cresceu <strong>{pct:.1f}%</strong> vs período anterior. "
            f"Mantendo tendência saudável."
        )
    if agendados > 0:
        insights.append(
...
def gerar_insight_barbearia(dados):
    ...
    if atendimentos > 0:
        insights.append(
            f"✂️ Ticket médio de <strong>{formatar_brl(ticket)}</strong> "
            f"em {atendimentos} atendimentos."
        )
    return " ".join(insights) if insights else "✅ Barbearia fluindo bem."


# ═══════════════════════════════════════════════════════════════
# GERADORES DE DADOS (com parametrização de período)
# ═══════════════════════════════════════════════════════════════
...
def gerar_dados_barbearia(mes_ref="Jun", ano_ref="2026"):
    ...
    for _ in range(600):
        ...
        dados.append({
            ...
            "Servico": np.random.choice(servicos), "Barbeiro": barbeiro,
...
def exportar_pdf(titulo_relatorio, periodo, kpis_dict, tabelas_dict, insights_list):
    ...
    s_subtitulo = ParagraphStyle(
        name="SubPulse", fontSize=12, textColor=colors.HexColor("#64748b"),
        alignment=TA_CENTER, spaceAfter=20, leading=14
    )
    s_secao = ParagraphStyle(
        name="Secao", fontSize=14, textColor=colors.HexColor("#047857"),
        fontName="Helvetica-Bold", spaceAfter=10, spaceBefore=16, leading=16
    )
    s_texto = ParagraphStyle(
        name="Texto", fontSize=10, textColor=colors.HexColor("#0f172a"),
        leading=14, spaceAfter=8
    )
    s_insight = ParagraphStyle(
        name="Insight", fontSize=10, textColor=colors.HexColor("#064e3b"),
    ...
    elementos.append(Paragraph(
        ...
        ParagraphStyle(name="Assinatura", fontSize=10,
                       textColor=colors.HexColor("#64748b"), alignment=TA_CENTER)
    ))

    elementos.append(PageBreak())

    # PÁGINA 2 — KPIs e Insights
...
def bloco_export(titulo_relatorio, periodo, dfs_excel, kpis_dict, tabelas_pdf, insights_list):
    ...
    with c1:
        ...
        st.download_button(
            label="📊 Baixar Excel",
            data=excel_bytes,
            file_name=f"pulse_{titulo_relatorio.lower().replace(' ', '_')}_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx",
    ...
    with c2:
        ...
        st.download_button(
            label="📄 Baixar PDF",
            data=pdf_bytes,
            file_name=f"pulse_{titulo_relatorio.lower().replace(' ', '_')}_{datetime.now().strftime('%Y%m%d_%H%M')}.pdf",
...
def tela_inicial():
    ...
    <div style="text-align:center; margin-top:32px; padding-top:20px;
                border-top:1px solid rgba(0, 255, 136, 0.15); color:#94a3b8; font-size:0.75rem;">
...
def dash_corretora():
    ...
    <div class="insight-box">
        💡 <strong>Insight de {mes}/{ano}</strong><br>
        {insight}
    </div>
    """, unsafe_allow_html=True)
    ...
    with t5:
        ...
        fig = go.Figure(go.Bar(
            ...
            marker_color=VERDE_NEON, text=top_clientes.values, textposition='outside',
    ...
    tabelas_export = {
        ...
        "Vencendo Proximos 30d": df[(df["Dias_Para_Renovacao"] >= 0) & (df["Dias_Para_Renovacao"] <= 30)][
...
def dash_contabil():
    ...
    tabelas_export = {
        ...
        "Clientes em Atraso": df[df["Status_Fechamento"] == "Atrasado"][["Empresa", "Regime_Tributario", "Dias_Para_Entrega"]],
...
def dash_clinica():
    ...
    with t1:
        fig = go.Figure(go.Bar(x=df_evo["Mes"], y=df_evo["Receita"], marker_color=VERDE_NEON,
                               name=ano))
        if df_evo_ant is not None and tipo_comp == "YoY":
            fig.add_trace(go.Scatter(
    ...
    tabelas_export = {
        "Atendimentos Realizados": df_export[["Data", "Paciente", "Procedimento", "Profissional", "Valor"]].head(100),
        "Evolucao Mensal": df_evo,
    }
...
def dash_barbearia():
    ...
    df_export["Data"] = df_export["Data"].dt.strftime("%d/%m/%Y %H:%M")
    tabelas_export = {
        "Atendimentos": df_export[["Data", "Cliente", "Servico", "Barbeiro", "Valor", "Comissao_Barbeiro"]].head(100),
        "Produtos Vendidos": df_prod[["Data", "Produto", "Quantidade", "Receita_Total", "Lucro_Bruto"]].head(100),
        "Evolucao Mensal": df_evo,
    }
